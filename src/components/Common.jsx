...
export function WorkCard({ work }) {
  return (
    <article className="card work-card">
      <div className="eyebrow">{work.work_type?.replaceAll('_',' ') || 'work'} {work.publication_year ? `• ${work.publication_year}` : ''}</div>
      <h3><Link to={`/work/${work.id}`}>{work.title}</Link></h3>
      <div className="muted">{[work.journal, work.doi ? `DOI ${work.doi}` : null].filter(Boolean).join(' • ')}</div>
    </article>
  );
}
...
export function RequireAuth({ children }) {
  return <div className="card"><h2>Sign in required</h2><p className="muted">Create a profile to contribute evidence, appraise studies, vote on competing extractions, create reviews, follow researchers, and join organizations.</p><Link to="/auth" className="button primary">Sign in or create profile</Link>{children}</div>;
}
